...
const StatCard: React.FC<StatCardProps> = ({ title, value, icon, description, color }) => (
  <div className={`p-5 rounded-lg shadow-md flex flex-col justify-between h-40 border-l-4 ${color} bg-[#1B2D3B] transition-shadow hover:shadow-lg`}>
    <div className="flex items-center justify-between">
      <h3 className="text-sm font-medium text-white uppercase tracking-wider">{title}</h3>
      {icon}
    </div>
    <p className="text-4xl font-bold text-white mt-2">{value.toLocaleString('pt-BR')}</p>
    <span className="text-xs text-white text-muted-foreground mt-1">{description}</span>
  </div>
);
    ...
    <div className="p-8">
      ...
      {!loading && resumo && (
        <>
            <section className="mb-10">
                <h2 className="text-2xl font-semibold mb-4 text-white">Frota e Veículos</h2>
            ...
            <section className="mb-10 ">
                <h2 className="text-2xl font-semibold mb-4 text-white">Estoque e Status dos Pneus</h2>
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
      ...
      <footer className="mt-8 text-white">
        ...
        <div className="flex flex-wrap gap-4">
            <Link to="/veiculos" className="flex items-center gap-2 p-3 bg-gray-100 rounded-lg text-gray-700 hover:bg-blue-200 transition">
                <Car className="size-5" /> Gerenciar Frota
            </Link>
            <Link to="/pneus" className="flex items-center gap-2 p-3 bg-gray-100 rounded-lg text-gray-700 hover:bg-blue-200 transition">
